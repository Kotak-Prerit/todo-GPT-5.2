...
import { FiCheck, FiChevronDown, FiChevronRight, FiPlus, FiTrash2 } from 'react-icons/fi'
...
function App() {
  ...
  const Checkbox = ({
    ...
    return (
      <button
        ...
        className={
          checked
            ? 'grid h-5 w-5 place-items-center rounded-md border border-zinc-400 bg-zinc-100 text-zinc-950'
            : 'grid h-5 w-5 place-items-center rounded-md border border-zinc-600 bg-zinc-950 text-transparent hover:border-zinc-400'
        }
      >
        <FiCheck size={14} />
      </button>
  ...
  const TodoNode = ({ todo, depth }: { todo: Todo; depth: number }) => {
    ...
    return (
      <div>
        <div
          className="group flex items-center gap-2 rounded-lg border border-zinc-900 bg-zinc-950/40 px-3 py-2"
        ...
        <div className="rounded-2xl border border-zinc-900 bg-zinc-950/50 p-4">
          ...
          <div className="mt-4 space-y-2">
            {visibleTodos.length === 0 ? (
              ...
              visibleTodos.map((t) => <TodoNode key={t.id} todo={t} depth={0} />)
            )}
          </div>

          <footer className="mt-5 flex items-center justify-between gap-3 border-t border-zinc-900 pt-4">
            <div className="text-xs text-zinc-500">Double-click a task to edit</div>
